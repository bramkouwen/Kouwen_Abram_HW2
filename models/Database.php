...
class Database {  // our base model class we will extend
	...
	public function getAll() {
		$stmt = $this->connection->prepare("SELECT users.id, users.lastname, users.firstname, users.username, users.password, users.photo, roles.rolename FROM users INNER JOIN roles ON users.role_id = roles.role_id ");
		$stmt->execute();
	...
	public function getOne($id) {
		$stmt = $this->connection->prepare("SELECT users.id, users.lastname, users.firstname, users.username, users.password, users.photo, roles.rolename FROM users INNER JOIN roles ON users.role_id = roles.role_id WHERE id = ?" );
		$stmt->execute([$id]);
		$arr = $stmt->fetchAll(PDO::FETCH_OBJ);
		if(!$arr) exit('No results returned.');
		return $arr;
